...
export function computeUiState({ i18n, facts, check }) {
  ...
  // Issue 5: always allow 'update' if an update is available, even when
  // a previous update is staged and pending reboot.
  ...
  if (facts.needsReboot && check?.phase === 'available') {
    // Both a staged update (pending reboot) AND a newer update exist.
    const size = check?.downloadSize || i18n.t('unknown');
    statusText = `${i18n.t('update_pending_newer_available')} ${i18n.t('download_size')}: ${size}.`;
