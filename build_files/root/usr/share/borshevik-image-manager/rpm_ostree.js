...
export function extractDigest(deployment) {
  // Try multiple fields in priority order. rpm-ostree versions expose
  // the image digest in different JSON fields.
  // 1. Explicit digest field
  const d = deployment?.['container-image-reference-digest']
    ?? deployment?.container_image_reference_digest;
  if (d) return d;

  // 2. Parse @sha256:... from the container image reference
  const ref = deployment?.['container-image-reference']
    ?? deployment?.container_image_reference
    ?? deployment?.origin ?? '';
  const m = ref.match(/@(sha256:[0-9a-fA-F]{64})/);
  if (m) return m[1];

  return null;
}

export function deriveVariantName(origin) {
  // Returns the workflow-compatible variant name ('borshevik' or 'borshevik-nvidia').
  const lower = (origin || '').toLowerCase();
  if (lower.includes('borshevik-nvidia'))
    return 'borshevik-nvidia';
  if (lower.includes('borshevik'))
    return 'borshevik';
  return null;
}
